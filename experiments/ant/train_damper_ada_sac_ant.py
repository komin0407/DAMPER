#!/usr/bin/env python3
"""DAMPER-Ada-SAC (PAVE/sac/models/damper_ada_sac.py) on Ant-v5.

Alignment-adaptive gradient surgery: the temporal gradient amplification is
set per update by its cosine alignment with the return gradient (aligned ->
norm-balance-like, conflicting -> cap-like). Everything else is identical to
train_damper_sac_ant.py.

Identical hyperparameters to the Pendulum/LunarLander/Reacher SAC runs
(train_damper_sac*.py): DAMPERAdaSAC (return-priority, alignment-adaptive
...
from sac.models.damper_ada_sac import DAMPERAdaSAC  # noqa: E402
...
@th.no_grad()
def evaluate_policy(model: DAMPERAdaSAC, validation_seeds: list[int]) -> list[dict[str, object]]:
    env = gym.make(ENV_ID)
