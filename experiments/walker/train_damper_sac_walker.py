#!/usr/bin/env python3
"""DAMPER-SAC (PAVE/sac/models/damper_sac.py) on Walker2d-v5.

Norm-balanced gradient surgery variant (the original DAMPER): the temporal
gradient is normalized to a unit vector and merged with the return gradient
at equal weight (after return-priority projection on conflict).

Identical hyperparameters to the Pendulum/LunarLander/Reacher SAC runs
(train_damper_sac*.py): DAMPERSAC (return-priority, norm-balanced
...
from sac.models.damper_sac import DAMPERSAC  # noqa: E402
...
@th.no_grad()
def evaluate_policy(model: DAMPERSAC, validation_seeds: list[int]) -> list[dict[str, object]]:
    env = gym.make(ENV_ID)
