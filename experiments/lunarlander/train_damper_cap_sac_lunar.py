#!/usr/bin/env python3
"""DAMPER-Cap-SAC (PAVE/sac/models/damper_cap_sac.py) on LunarLander-v3 (continuous).

Cap-only gradient surgery variant: the temporal gradient keeps its raw
magnitude, capped at the return norm (never amplified). Everything else is
identical to train_damper_sac_lunar.py.

Identical hyperparameters to the Pendulum SAC run: DAMPERCapSAC class
(return-priority, cap-only two-task surgery on the actor;
SiLU forced; net_arch [256,256] SB3 SAC default), same learning_rate=3e-4,
same SB3 SAC defaults for buffer_size/learning_starts/gamma/train_freq.
Only the environment and total_timesteps change (500_000 for LunarLander vs
100_000 for Pendulum, matching PAVE's LunarLander schedule).
"""
...
from sac.models.damper_cap_sac import DAMPERCapSAC  # noqa: E402
...
@th.no_grad()
def evaluate_policy(model: DAMPERCapSAC, validation_seeds: list[int]) -> list[dict[str, object]]:
