...
implementation), this script exercises the actual DAMPERTD3 class checked into the
...
Conditions matched to PAVE-TD3's own Pendulum settings:
  - SiLU activation (forced by DAMPERTD3.__init__)
...
@th.no_grad()
def evaluate_policy(env_id: str, model: DAMPERTD3, validation_seeds: list[int]) -> list[dict[str, object]]:
    env = gym.make(env_id)
