...
def _set_two_task_etagrad(
    ...
    cap-only and norm-balanced DAMPER variants.
...
class DAMPERUnifiedSAC(SAC):
    """Interpolated Return-Priority Temporal Gradient Surgery on SAC.

    Unifies DAMPERSAC (norm-balanced) and DAMPERCapSAC (cap-only) through a
    ...
    def __init__(
        ...
        print(f"[*] DAMPER eta: {self.eta} (0=cap-only, 1=norm-balanced)")
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            # -----------------------------------------------------------
            # [DAMPER-Unified Actor Update -- interpolated two-task surgery]
            # -----------------------------------------------------------
        ...
        # DAMPER-Unified diagnostics
        self.logger.record("train/damper_eta", self.eta)
        ...
        if len(cosines) > 0:
            ...
            self.logger.record("train/damper_w", np.mean(ws))
